...
def plot_system(A, title, filename_base, eigenvalues, eigenvectors):
    ...
    # Показываем собственные векторы (только вещественные)
    if eigenvectors is not None:
        for i, eigenvector in enumerate(eigenvectors):
            # Берем только вещественную часть для отображения
            real_eigenvector = np.real(eigenvector)
            if np.linalg.norm(real_eigenvector) > 1e-10:  # Проверяем, что вектор не нулевой
