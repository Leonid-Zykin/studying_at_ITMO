...
Эти результаты подтверждают, что тригонометрическая и комплексная формы рядов Фурье дают математически эквивалентные результаты с точностью до машинной точности вычислений.

\newpage 
\section*{Вывод}

В ходе работы изучены ряды Фурье в тригонометрической и комплексной формах. Для квадратной волны вычислены коэффициенты $a_n$, $b_n$, $c_n$ первых трёх гармоник. Программа на Python позволила получить коэффициенты при произвольном $N$, построить графики частичных сумм и проверить равенство Парсеваля.

Исследована комплекснозначная кусочная функция с расчётом коэффициентов $c_n$ при $n = 0, 1, 2$. Построены параметрические графики исходной функции и аппроксимаций $G_N(t)$, визуализированы действительная и мнимая части, проверена теорема Парсеваля.

Результаты показывают эффективность рядов Фурье для приближения периодических функций элементарными гармониками.
