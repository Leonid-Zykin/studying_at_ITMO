...
def apply_transform(vertices, matrix):
    ...
    # Применяем преобразование (без транспонирования)
    transformed = matrix @ homogeneous.T
    # Возвращаем к обычным координатам и транспонируем обратно
...
# 6. Вращение вокруг вершины - НАЙДЕМ МАТРИЦУ ПРЕОБРАЗОВАНИЯ

def rotation_around_vertex_matrix(vertex, theta, axis_vector):
    """Находим матрицу преобразования для поворота вокруг вершины"""
    # Шаг 1: Матрица переноса T (перемещаем вершину в начало координат)
    T = np.array([
        [1, 0, 0, -vertex[0]],
        [0, 1, 0, -vertex[1]],
        [0, 0, 1, -vertex[2]],
    ...
    # Шаг 2: Матрица поворота R вокруг оси
    R = rotation_axis_matrix(axis_vector, theta)
    
    # Шаг 3: Обратная матрица переноса T^(-1)
    T_inv = np.array([
        [1, 0, 0, vertex[0]],
        [0, 1, 0, vertex[1]],
        [0, 0, 1, vertex[2]],
    ...
    # Искомая матрица: M = T^(-1) * R * T
    M = T_inv @ R @ T
    return M

def rotate_around_vertex(vertices, vertex_idx, theta, axis_vector):
    """Поворот куба вокруг вершины используя найденную матрицу"""
    vertex = vertices[vertex_idx]
    M = rotation_around_vertex_matrix(vertex, theta, axis_vector)
...
def generate_all_images():
    ...
    ax.set_title('Задание 1: Пример тетраэдра в 3D пространстве')
    ax.legend()
    ...
    ax1.set_title('Задание 2: Исходный куб')
    ...
    ax2.set_title('Задание 2: Масштабированный куб\n(2, 1.5, 0.5)')
    ...
    ax3.set_title('Задание 2: Уменьшенный куб\n(0.5, 0.5, 0.5)')
    ...
    ax1 = fig.add_subplot(131, projection='3d')
    plot_cube(ax1, vertices, faces, 'blue', 0.5)
    ax1.set_title('Задание 3: Исходный куб')
    set_axes_equal(ax1, vertices)
    ...
    ax2.set_title('Задание 3: Перенесенный куб\n(3, 2, 1)')
    ...
    ax3.set_title('Задание 3: Комбинированное TS\n(перенос + масштаб)')
    ...
    ax1.set_title('Задание 3: TS (перенос, затем масштаб)')
    set_axes_equal(ax1, ts_vertices)
    
    ax2 = fig.add_subplot(132, projection='3d')
    plot_cube(ax2, st_vertices, faces, 'purple', 0.5)
    ax2.set_title('Задание 3: ST (масштаб, затем перенос)')
    ...
    ax3.set_title('Задание 3: Сравнение TS и ST')
    ax3.legend()
    ...
    ax1 = fig.add_subplot(131, projection='3d')
    plot_cube(ax1, vertices, faces, 'blue', 0.5)
    ax1.set_title('Задание 4: Исходный куб')
    set_axes_equal(ax1, vertices)
    ...
    ax2.set_title(f'Задание 4: Поворот вокруг оси {v}\nна угол {theta:.2f} рад')
    ax2.legend()
    ...
    ax3.set_title('Задание 4: Rx(π/4)Ry(π/4)Rz(π/4)')
    ...
    ax1.set_title('Задание 5: Поворот куба вокруг вершины 0\nна угол π/3 вокруг оси Z')
    ax1.legend()
    ...
    ax2.set_title('Задание 5: Поворот куба вокруг вершины 0\nна угол π/2 вокруг оси Z')
    ax2.legend()
    ...
    for i, (dx, dy, dz) in enumerate(positions):
        ...
        plot_cube(ax1, cube_vertices, faces, colors[i], 0.5)
    ax1.set_title('Задание 6: Исходная сцена с несколькими кубами')
    # Собираем все вершины для правильного масштабирования
    all_scene_vertices = []
    for dx, dy, dz in positions:
        T_cube = translation_matrix(dx, dy, dz)
        cube_vertices = apply_transform(vertices, T_cube)
        all_scene_vertices.append(cube_vertices)
    all_scene_vertices = np.vstack(all_scene_vertices)
    set_axes_equal(ax1, all_scene_vertices)
    
    # Применяем камеру
    camera_transform_matrix = camera_matrix(2, 2, 2, np.pi/6, np.pi/6, 0)
    camera_vertices = apply_transform(all_scene_vertices, camera_transform_matrix)
    
    ax2 = fig.add_subplot(122, projection='3d')
    for i, (dx, dy, dz) in enumerate(positions):
        T_cube = translation_matrix(dx, dy, dz)
        cube_vertices = apply_transform(vertices, T_cube)
        camera_cube_vertices = apply_transform(cube_vertices, camera_transform_matrix)
        plot_cube(ax2, camera_cube_vertices, faces, colors[i], 0.5)
    ax2.set_title('Задание 6: Сцена после применения камеры\n(позиция: 2,2,2, поворот: π/6,π/6,0)')
    ...
    perspective_vertices = apply_perspective(all_scene_vertices, fov=60, near=0.1, far=100)
    ...
    for i, (dx, dy, dz) in enumerate(positions):
        ...
        cube_vertices = apply_transform(vertices, T_cube)
        plot_cube(ax1, cube_vertices, faces, colors[i], 0.5)
    ax1.set_title('Задание 7: Сцена без перспективы')
    set_axes_equal(ax1, all_scene_vertices)
    ...
    for i, (dx, dy, dz) in enumerate(positions):
        T_cube = translation_matrix(dx, dy, dz)
        cube_vertices = apply_transform(vertices, T_cube)
        perspective_cube_vertices = apply_perspective(cube_vertices, fov=60, near=0.1, far=100)
        plot_cube(ax2, perspective_cube_vertices, faces, colors[i], 0.5)
    ax2.set_title('Задание 7: Сцена с перспективной проекцией\n(fov=60°, near=0.1, far=100)')
    ...
    for i, (dx, dy, dz) in enumerate(positions):
        ...
        cube_vertices = apply_transform(vertices, T_cube)
        perspective_cube_vertices = apply_perspective(cube_vertices, fov=60, near=0.1, far=100)
        plot_cube(ax, perspective_cube_vertices, faces, colors[i], 0.5)
    ax.set_title('Задание 7: Перспективная проекция - вид сбоку')
